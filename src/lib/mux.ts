...
type MuxEvent = { id: string; type: string; data: { id?: string; upload_id?: string; playback_ids?: Array<{ id: string; policy: string }>; errors?: { messages?: string[] } } };
...
export async function createDirectUpload({ uploadId, origin }: { uploadId: string; origin: string }) {
  const response = await fetch(`${MUX_API}/uploads`, {
    ...
    body: JSON.stringify({
      cors_origin: origin,
      timeout: 3600,
      new_asset_settings: { playback_policies: ["signed"], passthrough: uploadId, video_quality: "basic" },
    }),
    cache: "no-store",
  });
  if (!response.ok) throw new Error(`Mux upload creation failed (${response.status})`);
  return (await response.json() as { data: MuxUpload }).data;
}

export function parseMuxSignature(value: string) {
  return Object.fromEntries(value.split(",").map((part) => part.trim().split("=", 2)).filter(([key, item]) => key && item));
}

export function verifyMuxWebhook(body: string, signature: string, secret: string, now = Date.now()) {
  const values = parseMuxSignature(signature);
  const timestamp = Number(values.t);
  if (!values.v1 || !Number.isFinite(timestamp) || Math.abs(now - timestamp * 1000) > 300_000) return false;
  const expected = createHmac("sha256", secret).update(`${values.t}.${body}`).digest("hex");
...
export function createPlaybackToken(playbackId: string, expiresInSeconds = 3600, now = Math.floor(Date.now() / 1000)) {
  ...
  const payload = encode({ sub: playbackId, aud: "v", exp: now + expiresInSeconds, kid: keyId });
  const unsigned = `${header}.${payload}`;
  const signature = createSign("RSA-SHA256").update(unsigned).sign(createPrivateKey(privateKey), "base64url");
