...
describe("Mux security adapter", () => {
  ...
  it("parses and verifies a current Mux webhook signature", () => {
    ...
    expect(parseMuxSignature(signature)).toEqual({
      t: String(timestamp),
      v1: digest,
    });
    expect(verifyMuxWebhook(body, signature, "secret", timestamp * 1000)).toBe(
      true,
    );
    expect(verifyMuxWebhook(body, signature, "wrong", timestamp * 1000)).toBe(
      false,
    );
  });
  ...
  it("creates a verifiable short-lived RS256 playback token", () => {
    ...
    ).toMatchObject({ sub: "playback-123", aud: "v", exp: 1_700_000_900 });
    expect(
      verify(
        "RSA-SHA256",
  ...
  it("cancels an active direct upload through the trusted adapter", async () => {
    process.env.MUX_TOKEN_ID = "token-id";
    process.env.MUX_TOKEN_SECRET = "token-secret";
    const fetchMock = vi.fn().mockResolvedValue(new Response(null));
    vi.stubGlobal("fetch", fetchMock);

    await cancelDirectUpload("direct/upload");

    expect(fetchMock).toHaveBeenCalledWith(
      "https://api.mux.com/video/v1/uploads/direct%2Fupload/cancel",
