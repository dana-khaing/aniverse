  ...
  [
    {
      ...
      detail: "New episodes, premieres, and schedule changes.",
    },
    {
      key: "communityEmail",
      title: "Community emails",
      detail: "Replies, reactions, follows, and moderation updates.",
    },
    {
      key: "creatorEmail",
      title: "Creator emails",
      detail: "Publishing, team invitations, uploads, and payouts.",
    },
    {
      key: "pushEnabled",
      title: "Push notifications",
      detail:
        "Release, reply, moderation, and creator alerts on subscribed devices.",
    },
    {
      key: "inAppEnabled",
      title: "In-app notifications",
      detail: "Show activity in AniVerse while signed in.",
    },
  ];

export function NotificationSettings() {
  const [settings, setSettings] = useLocalDemoState<Settings>(
    "aniverse.notification-settings",
    initial,
  );
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      setBusy(true);
      void fetch("/api/v1/account/notifications", { cache: "no-store" })
        .then(async (response) =>
  ...
  async function save(event: React.FormEvent) {
    ...
    if (cloud) {
      ...
      if (!response.ok) {
        setMessage(data.error ?? "Notification settings could not be saved.");
        setBusy(false);
        return;
      }
    }
      ...
      <form className="settings-card" onSubmit={(event) => void save(event)}>
        <header>
          ...
          <div>
            ...
            <small>
              Preferences are private and scoped to this signed-in account.
            </small>
          </div>
        </header>
        <div className="settings-options">
          {channels.map((channel) => (
            <label className="settings-toggle" key={channel.key}>
              <span>
                <b>{channel.title}</b>
