...
vi.mock("@/lib/local-data/use-indexed-records", () => ({ useIndexedRecords: () => ({ records: [], loading: false, refresh: vi.fn() }) }));
vi.mock("@/components/creator/creator-insights", () => ({ CreatorInsights: () => null }));
...
describe("creator workspace", () => {
  ...
  it("creates titles and episodes in local fallback mode", () => {
    ...
    fireEvent.change(screen.getByLabelText("New title name"), { target: { value: "Moon Garden" } });
