...
export function CreatorWorkspace() {
  ...
  return <div className="studio-layout">
    ...
    <div className="studio-content">
      ...
      <section id="overview" className="metric-grid"><article><b>{workspace.titles.length}</b><span>Titles</span></article><article><b>{workspace.titles.reduce((sum,title)=>sum+title.episodes,0)}</b><span>Episodes</span></article><article><b>{workspace.uploads.length}</b><span>Assets</span></article><article><b>{workspace.team.members.length}</b><span>Team members</span></article></section>
      ...
      <section id="uploads" className="studio-panel"><div className="panel-head"><div><p>VIDEO PIPELINE</p><h2>Uploads and subtitles</h2></div><label className="upload-button"><Upload size={15}/>Upload video<input type="file" accept="video/*" onChange={(event)=>{const file=event.target.files?.[0];if(!file)return;setWorkspace((current)=>({...current,uploads:[{id:crypto.randomUUID(),filename:file.name,title:current.titles[0]?.name??"Untitled",status:"Ready",subtitles:["English CC"]},...current.uploads]}));event.target.value="";}}/></label></div>{workspace.uploads.length ? <div className="upload-list">{workspace.uploads.map((upload)=><article key={upload.id}><FileVideo/><div><b>{upload.filename}</b><span>{upload.title} · {upload.subtitles.join(", ")}</span></div><i>{upload.status}</i></article>)}</div>:<div className="studio-empty"><FileVideo/><h3>No video assets yet</h3><p>Upload a local video to simulate ingest, transcoding, and subtitle attachment.</p></div>}</section>
