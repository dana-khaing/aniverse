...
export async function GET() {
  ...
  const { data, error } = await admin
    ...
    .select(
      "id,user_id,channel_name,legal_name,portfolio_url,rights_summary,status,submitted_at,reviewed_at,review_notes",
    )
    .in("status", ["submitted", "approved", "rejected"])
    .order("submitted_at", { ascending: false });
  if (error)
    return Response.json(
      { error: "Could not load creator queue" },
      { status: 500 },
    );
  ...
  return Response.json(
    {
      applications: (data ?? []).map((item) => ({
        ...item,
        applicant_name: names.get(item.user_id) ?? "Applicant",
      })),
    },
...
export async function PATCH(request: Request) {
  ...
  const { data: application } = await admin
    .from("creator_applications")
    .select("id,user_id,channel_name,status")
  ...
  if (parsed.data.decision === "approved") {
    ...
    if (!team) {
      const result = await admin
        ...
        .insert({
          ...
          slug: creatorSlug(application.channel_name, application.id),
      ...
      if (result.error)
        return Response.json(
          { error: "Could not provision creator team" },
          { status: 500 },
        );
      team = result.data;
    ...
    if (membershipError) {
      if (createdTeamId)
        await admin.from("creator_teams").delete().eq("id", createdTeamId);
      return Response.json(
        { error: "Could not provision team membership" },
        { status: 500 },
      );
    }
    ...
    if (roleError)
      return Response.json(
        { error: "Could not grant creator role" },
        { status: 500 },
      );
  }
  const { data, error } = await admin
  ...
  await admin
    ...
    .insert({
      ...
      action: `creator_application.${parsed.data.decision}`,
      entity_type: "creator_application",
      entity_id: application.id,
      metadata: { applicant_id: application.user_id },
    });
  await admin.from("notifications").insert({
    user_id: application.user_id,
    type: "creator_application",
    title: `Creator application ${parsed.data.decision}`,
    body:
      parsed.data.notes ||
      `Your creator application has been ${parsed.data.decision}.`,
    href: "/creator/apply",
  });
