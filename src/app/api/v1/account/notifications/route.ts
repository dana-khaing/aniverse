...
async function identity() {
  if (!isSupabaseConfigured())
    return {
      error: Response.json(
        { error: "Cloud notification settings are unavailable" },
        { status: 503 },
      ),
    };
...
export async function GET() {
  ...
  const { data, error } = await access.supabase
    ...
    .select(
      "release_email,community_email,creator_email,push_enabled,in_app_enabled",
    )
  ...
  return Response.json(
    {
      settings: {
        releaseEmail: data.release_email,
...
export async function PATCH(request: Request) {
  ...
  if (origin && origin !== new URL(request.url).origin)
    return Response.json(
      { error: "Untrusted settings origin" },
      { status: 403 },
    );
  ...
  const { error } = await access.supabase
    .from("notification_preferences")
    .update({
      release_email: parsed.data.releaseEmail,
      community_email: parsed.data.communityEmail,
      creator_email: parsed.data.creatorEmail,
